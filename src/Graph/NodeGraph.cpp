...
usagi::node_graph::Port::Port(std::string name, Edge edge, float edge_pos)
    : name(std::move(name)), edge(edge), edge_pos(edge_pos) {}

usagi::node_graph::NodePrototype::NodePrototype(std::string name, Vector2f size)
    : name(std::move(name)), size(std::move(size)) {}

usagi::node_graph::NodePrototype::NodePrototype(std::string name, Vector2f size, std::size_t in, std::size_t out)
    : NodePrototype(name, size) {
    createInPorts(in);
    createOutPorts(out);
}

void usagi::node_graph::NodePrototype::createPorts(std::vector<Port> &sink, std::size_t amount, Port::Edge edge) {
    sink.resize(amount);
...
usagi::Vector2f usagi::node_graph::NodePrototype::portPosition(const Port &p, const Vector2f &position) const {
    Vector2f pos;
    switch (p.edge) {
        case Port::Edge::NORTH:
            pos.x() = lerp(p.edge_pos, 0.f, size.x()) + position.x();
            pos.y() = position.y();
            break;
        ...
        case Port::Edge::WEST:
            pos.x() = position.x();
            pos.y() = lerp(p.edge_pos, 0.f, size.y()) + position.y();
            break;
        case Port::Edge::EAST:
...
usagi::node_graph::Node::Node(NodePrototype *prototype, std::string name)
    : prototype(prototype), name(std::move(name)) {}

usagi::node_graph::Link::Link(std::size_t node0, std::size_t port0, std::size_t node1, std::size_t port1)
    : node0(node0), port0(port0), node1(node1), port1(port1) {}

std::tuple<const usagi::node_graph::Node &, const usagi::node_graph::Port &, const usagi::node_graph::Node &,
           const usagi::node_graph::Port &>
usagi::node_graph::NodeGraph::mapLink(std::size_t i) const {
    auto &l = link(i);
    auto &n0 = node(l.node0);
    auto &n1 = node(l.node1);
    auto &p0 = n0.prototype->outPort(l.port0);
    auto &p1 = n1.prototype->inPort(l.port1);
    return std::make_tuple(std::ref(n0), std::ref(p0), std::ref(n1), std::ref(p1));
}

usagi::node_graph::NodeGraph usagi::node_graph::NodeGraph::readFromFile(const std::filesystem::path &path) {
...
std::tuple<usagi::Vector2f, usagi::Vector2f> usagi::node_graph::NodeGraphInstance::mapLinkEndPoints(
    std::size_t i) const {
...
usagi::AlignedBox2f usagi::node_graph::NodeGraphInstance::mapNodeRegion(std::size_t node_index) const {
    const auto node_position = mapNodePosition(node_index);
    return {node_position, node_position + base_graph->node(node_index).prototype->size};
}
